import { Add_Todo, Edit_Todo, Done_Undone_Todo, Delete_Todo } from './actionsType';
...
function TodoReducer (state = init, action) {
    switch(action.type){
        ...
        case Edit_Todo :
            return {...state, tasks : state.tasks.map((task) => task.id === action.payload.id ? action.payload
                 : task)}
        default :
            return state;
    }
